...
$comando = "update `$linha[1]`.`vales` set `codigo` = '$codigo',`data` = '$data',`dia` = '$dia',`mes` = '$mes',`ano` = '$ano',`valor_vale` = '$valor_vale',`nome` = '$nome',`obs` = '$obs' where `vales`. `codigo` = '$codigo' limit 1 ";

}



mysql_query($comando,$conexao) or die("Erro ao alterar informações do registro de adiantamentos do funcionário $nome");
...
<p>
  <table width="40%" border="1" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td width="47%"><div align="center">Valor do adiantamento</div></td>
      <td width="53%"><div align="center">Adiantamento retirado em</div></td>
    </tr>
    <tr>
...
function extenso($valor_vale = 0, $maiusculas = false) {
...
$valor_vale = number_format($valor_vale, 2, ".", ".");
$inteiro = explode(".", $valor_vale);
...
for ($i=0;$i<count($inteiro);$i++) {
$valor_vale = $inteiro[$i];
$rc = (($valor_vale > 100) && ($valor_vale < 200)) ? "cento" : $c[$valor_vale[0]];
$rd = ($valor_vale[1] < 2) ? "" : $d[$valor_vale[1]];
$ru = ($valor_vale > 0) ? (($valor_vale[1] == 1) ? $d10[$valor_vale[2]] : $u[$valor_vale[2]]) : "";

$r = $rc.(($rc && ($rd || $ru)) ? " e " : "").$rd.(($rd &&
$ru) ? " e " : "").$ru;
$t = count($inteiro)-1-$i;
$r .= $r ? " ".($valor_vale > 1 ? $plural[$t] : $singular[$t]) : "";
